...
process RUN_METAPOINTFINDER{
    ...
    publishDir "${params.output}/${climb_id}/metapointfinder", mode: 'copy'
    maxForks 4
    ...
    output:
        tuple path("*.prot.updated_table_with_scores_and_mutations.tsv"), path("*.dna.updated_table_with_scores_and_mutations.tsv"), path("*.class.prot.summary.txt"), path("*gene.prot.summary.txt"), path("*.class.dna.summary.txt"), path("*gene.dna.summary.txt"), emit: metapointfinder_out
        tuple path("*.error"), path("*.log"), emit: metapointfinder_logs 
