...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument("-e", "--email",
                help="Email address to query Enterez.",
...
# Summaries & plots
# -------------------------x
def summarize_by_class(df, unique_resistance_classes):
...
def heatplot(df: pd.DataFrame, output_path: str):
    ...
    pivot = (df.groupby(['GENE', 'species_name'])['SEQUENCE']
            .nunique()
...
def most_common(
    ...
    round_dp: int = 2
) -> Union[Tuple[object, int, float], List[Tuple[object, int, float]]]:
    """
    ...
    vals = [
        (idx, int(cnt), round((cnt / total) * factor, round_dp))
...
def sankey_html_from_counts(
    ...
    node = dict(label=labels, pad=12, thickness=18)
    link = dict(
...
def coocc_heatmap_div(
    ...
    fig.update_layout(
        title=None,
        xaxis_title="Class", yaxis_title="Class",
...
def read_amr_summary(df: pd.DataFrame, unique_resistance_classes: list, output_path: str) -> dict:
    ...
    per_seq = pd.concat(
        [per_seq_hits, per_seq_genes, per_seq_classes, per_seq_class_presence],
        axis=1
    ).reset_index()
    ...
    hist_classes = per_seq['n_classes'].value_counts().sort_index().rename_axis('n_classes').reset_index(name='n_sequences')
