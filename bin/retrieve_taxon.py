...
def load_files(tsv_fp: Path, json_fp: Path) -> tuple[pd.DataFrame, dict]:
    """
    Load in tsv file as
    """
...
def add_species(df_tsv: pd.DataFrame, taxid_dict: dict) -> pd.DataFrame:
    """
    ...
    df_merged = df_tsv.merge(taxid_df[["taxid", "name", "raw_rank", "rank"]], on="taxid", how="left")
    return df_merged
